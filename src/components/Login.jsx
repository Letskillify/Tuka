...
const Login = () => {
  const { sendOtp, verifyOtp, loginWithPassword, loginWithGoogle, setAccountPassword, user } = useAuth();
  ...
  // OTP Steps: 1 (Email) -> 2 (Enter OTP) -> 3 (Create Password)
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      if (res.isNewUser || !res.hasPassword) {
        setStep(3);
        setSuccessMsg("Email verified! You can now optionally create a password for your account.");
      } else {
    ...
    <div className="min-h-screen flex selection:bg-[#b13896] selection:text-white bg-[#161114]">
      ...
      <div className="hidden lg:flex lg:w-[48%] relative overflow-hidden items-center justify-center p-12 bg-[#161114] border-r border-white/10">
        ...
        >
          ...
          <h2 className="text-4xl xl:text-5xl font-light text-white mb-6 leading-[1.15] tracking-tight" style={{ fontFamily: SERIF }}>
            Welcome to<br />
            <span className="font-semibold text-[#f4cfeb]">House of Tuka</span>
          </h2>

          <p className="text-white/60 text-sm max-w-md leading-relaxed mb-10 font-sans font-light">
            Sign in with email OTP, password, or Google to manage your orders, access saved favorites in your wishlist, and explore authentic Bengal handloom collections.
          </p>

          <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-8 font-sans">
            {[
              { title: 'Email OTP Auth', desc: 'Secure passwordless verification' },
              { title: 'Wishlist Sync', desc: 'Save your favorite drapes' },
      ...
      <div className="flex-1 flex flex-col justify-between bg-[#FDFAF5] px-6 sm:px-12 py-8 sm:py-10 relative min-h-screen">
        ...
        <div className="w-full flex items-center justify-between z-20">
          <Link
            to="/"
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#161114] text-white text-xs font-bold uppercase tracking-wider hover:bg-[#b13896] transition-all duration-300 shadow-md group cursor-pointer"
        ...
        <div className="w-full max-w-[430px] mx-auto my-auto py-6 relative z-10 font-sans">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            {/* Header with horizontal badge accent */}
            <div className="mb-8">
              ...
              <h1 className="text-4xl sm:text-5xl font-light text-[#161114] mb-2 leading-tight" style={{ fontFamily: SERIF }}>
                Sign <span className="italic text-[#b13896] font-normal">In</span>
              </h1>
            ...
            {/* Segmented Tab Pill Selector (EMAIL OTP vs PASSWORD) */}
            ...
            {authMode === "otp" && (
              <>
                  ...
                  <form onSubmit={handleSendOtp} className="space-y-5">
                    <div className="space-y-2">
                      ...
                      <div className="relative group">
                        <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-[#a3948b] group-focus-within:text-[#b13896] transition-colors" size={18} />
                        <input
                          type="email"
                          required
                          value={email}
                          ...
                          className="w-full pl-12 pr-4 py-3.5 bg-white border border-[#e6ded8] rounded-2xl focus:border-[#b13896] focus:ring-2 focus:ring-[#b13896]/15 outline-none transition-all text-xs text-[#161114] placeholder:text-[#a3948b] shadow-xs"
                        />
                      </div>
                  ...
                  <form onSubmit={handleVerifyOtp} className="space-y-6">
                      ...
                      <div className="flex justify-between gap-2" onPaste={handleOtpPaste}>
                        {otpDigits.map((digit, index) => (
                          <input
                            ...
                            className="w-12 h-14 text-center font-mono text-xl font-bold bg-white border border-[#e6ded8] rounded-2xl focus:border-[#b13896] focus:ring-2 focus:ring-[#b13896]/20 outline-none transition-all shadow-xs text-[#161114]"
                          />
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between text-xs pt-1 font-serif">
                      <button
                        ...
                        className="text-slate-500 hover:text-[#161114] underline"
                      >
                        Change Email
                      </button>
                ...
                {/* STEP 3: Create Password (Optional/New User) */}
                {step === 3 && (
                  <form onSubmit={handleSavePassword} className="space-y-5">
                    <div className="space-y-2">
                      ...
                      <div className="relative group">
                        ...
                        <button
                          ...
                          className="absolute right-4 top-1/2 -translate-y-1/2 text-[#a3948b] hover:text-[#b13896]"
                        >
                    ...
                    <div className="flex gap-3 pt-2">
                      <button
                        ...
                        className="flex-1 py-3.5 border border-[#e6ded8] rounded-2xl text-xs font-bold uppercase tracking-wider text-slate-600 hover:bg-[#F8F4EF]"
                      >
                        Skip
                      </button>
                      <button
                        type="submit"
                        disabled={loading}
                        className="flex-1 py-3.5 rounded-2xl bg-[#b13896] hover:bg-[#972d7f] text-white text-xs font-bold uppercase tracking-wider shadow-md flex items-center justify-center gap-2"
                      >
                        {loading ? <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : "Save & Continue"}
                      </button>
                    </div>
                  </form>
                )}
            ...
            {authMode === "password" && (
              <form onSubmit={handlePasswordLogin} className="space-y-5">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-4 rounded-2xl text-white font-bold text-xs tracking-[0.2em] uppercase transition-all bg-[#161114] hover:bg-[#b13896] shadow-md flex items-center justify-center gap-3 cursor-pointer mt-6"
                >
                  {loading ? (
                    <div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin" />
                  ) : (
            ...
            <div className="mt-8 text-center text-xs font-serif">
              <span className="text-[#86756c]">New to House of Tuka? </span>
              <Link to="/signup" className="text-[#161114] font-semibold underline hover:text-[#b13896]">
                Create an account
              </Link>
            </div>
